...
function sendForm($fname,$lname,$email,$country){
    ...
    if(!empty($row['subs_email'])){
        ...
        return send_email($fname, $email, true);
        exit;
        ...
        /*$replace_email_query = 'REPLACE INTO tbl_subs(subs_fname,subs_lname,subs_country,subs_last_update)';
        ...
        if($create_subs_set){
			return send_email($fname, $email, false);
		}else{
